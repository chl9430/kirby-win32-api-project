...
int GameSound::Load(const wstring& _strPath)
{
	if (nullptr == GameSoundMgr::GetInst()->GetSoundDevice())
...
bool GameSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(GameSoundMgr::GetInst()->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
...
void GameSound::Play(bool _bLoop)
{
...
void GameSound::PlayToBGM(bool _bLoop)
{
	GameSoundMgr::GetInst()->RegisterToBGM(this);
...
void GameSound::Stop(bool _bReset)
{
...
void GameSound::SetVolume(float _fVolume)
{
	m_iVolume = GetDecibel(_fVolume);
	m_pSoundBuffer->SetVolume(m_iVolume);
}

void GameSound::SetPosition(float _fPosition)
{
	Stop(true);
...
int GameSound::GetDecibel(float _fVolume)
{
	if (_fVolume > 100.f)
		_fVolume = 100.f;
